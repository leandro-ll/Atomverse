...
document.querySelectorAll('.inv-btn').forEach(btn => {
    btn.addEventListener('click', async function() {
        ...
        if (!outputDiv) {
            ...
            console.error('Periksa kembali HTML anda');
            alert('Error: Output container tidak ditemukan. Periksa console untuk detail.');
            return;
        }
        ...
        try {
            ...
            outputDiv.innerHTML = `
                <div class="p-4 bg-red-50 border border-red-200 rounded-lg text-red-700">
                    ...
                    <div class="text-xs mt-2">Pastikan backend uvicorn berjalan di http://127.0.0.1:8000</div>
                </div>
            `;
        }
    });
